...
# Set this to 1 to show the graph display in a separate VTK window. NOTE: We
# *always* want to leave this option enabled on OS X, since there it is not
# possible to embed the VTK widget in a Gnocl application.
if [expr ![info exists GRAPHWIN]] {set GRAPHWIN 0}
...
proc bbox {points} {
    # Yeah, Tcl is clumsy. This would be a two-liner in Lua.
...
if {[info exists prefix]} {
    set fname [file join $prefix "scale.html"]
    if {[file exists $fname]} {
	set helpfile $fname
    }
}
...
proc init_list {} {
    ...
    set list [gnocl::tree \
      ...
      -onRowCollapsed {lua collapse_cb %p} -onRowExpanded {lua expand_cb %p}]
...
#	       -value 1 -onToggled {lua interactor_cb %v} \
#	       -tooltip "Trackball/joystick mouse mode"] \
#    -expand 1 -fill 0 -align center
set drawbut [gnocl::button -text %__Draw -onClicked {lua draw_cb} \
		 -icon %#Refresh \
...
$rbox add [gnocl::button -text "%_Cl_ear" \
	       -onClicked {lua unselect_all_cb} \
	       -tooltip "Clear the selection"] \
    -expand 1 -fill 0 -align center
$rbox add [gnocl::button -text "%_Select _All" \
	       -onClicked {lua select_all_cb} \
	       -tooltip "Select all tuning alternatives"] \
    -expand 1 -fill 0 -align center
$rbox add [gnocl::button -text "%_Re_fresh" -onClicked {lua refresh_cb} \
...
set ratbut [gnocl::button -text "%_Ra_tionalize" \
		-onClicked {lua rat_cb} \
	        -tooltip "Rationalize scale"]
$rbox add  $ratbut -expand 1 -fill 0 -align center
$rbox add [gnocl::button -text %__Update -onClicked {lua update_cb} \
...
$tbox add [gnocl::spinButton -variable dmax -digits 2 \
	       -onValueChanged {lua dmax_cb %v} \
	       -tooltip "Weight threshold"] 1 0 -expand 0
$tbox add [gnocl::button -text %#Help -onClicked {lua help_cb} \
...
$bbox add [gnocl::button -text %#New  -onClicked {lua new_cb} \
	       -tooltip "New scale"]
$bbox add [gnocl::button -text %#Open -onClicked {lua open_cb} \
	       -tooltip "Open scale file"]
$bbox add [gnocl::button -text %#Save -onClicked {lua save_cb} \
	       -tooltip "Save scale file"]
$bbox add [gnocl::button -text %#RevertToSaved -onClicked {lua revert_cb} \
	       -tooltip "Revert scale file"]
$bbox add [gnocl::button -text %_A_bout -onClicked {lua about_cb} \
...
# XXXFIXME: This only works if the graph window is not embedded?
# Also it seems that there is a default key binding for <Escape> which
# terminates the program?
bind .embed <Key> {lua key_cb %s %K}
